...
fn main() {
    ...
    let block_display_thread = thread::spawn(move || {
        ...
        let _ = std::fs::write(
            log_file_path,
            "Blockchain Blok Görüntüleyici - Yeni bloklar burada görüntülenecek...\n",
        );
        ...
        while let Ok(message) = block_receiver.recv() {
            ...
            println!(
                "Yeni blok oluşturuldu: {} (Node {})",
                message.block.hash, message.validator_id
            );
    ...
    // Blockchain ağını başlat
    {
        ...
        match network_lock.start_automatic_mining() {
            Ok(_) => println!(
                "Otomatik madencilik başlatıldı. Block time: {} saniye",
                network_lock.block_time
            ),
            Err(e) => {
        ...
        match parts[0] {
            "bakiye" => {
                ...
                if let Ok(node_id) = parts[1].parse::<usize>() {
                    ...
                    if node_id >= network_lock.node_count() {
                        println!(
                            "Hata: Geçersiz Node ID. 0-{} arasında bir değer girin.",
                            network_lock.node_count() - 1
                        );
                        continue;
                    }
            ...
            "transfer" => {
                ...
                if let (Ok(sender_id), Ok(receiver_id), Ok(amount)) = (
                    ...
                    if sender_id >= network_lock.node_count()
                        || receiver_id >= network_lock.node_count()
                    {
                        println!(
                            "Hata: Geçersiz Node ID. 0-{} arasında bir değer girin.",
                            network_lock.node_count() - 1
            ...
            "mempool" => {
                ...
                if network_lock.mempool.is_empty() {
                    ...
                    for (i, tx) in network_lock.mempool.iter().enumerate() {
                        ...
                        println!(
                            "Miktar: {} coin",
                            tx.outputs[0].amount as f64 / 100_000_000.0
                        );
                    }
                }
                println!("-----------------------------\n");
